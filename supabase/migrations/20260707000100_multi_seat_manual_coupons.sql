-- Multi-seat manual coupons. Existing coupons stay one-seat coupons.

alter table public.ticket_coupons
  add column if not exists redeem_quantity int not null default 1;

alter table public.ticket_coupons
  drop constraint if exists ticket_coupons_redeem_quantity_check;

alter table public.ticket_coupons
  add constraint ticket_coupons_redeem_quantity_check
  check (redeem_quantity between 1 and 20);

drop function if exists public.create_ticket_coupons (uuid, uuid, text[], text[]);
...
create or replace function public.create_ticket_coupons (
  ...
  p_redeem_quantity int default 1
)
returns table (
  id uuid,
  code_hash text,
...
declare
  ...
  v_available int := 0;
  v_redeem_quantity int := coalesce(p_redeem_quantity, 1);
...
begin
  ...
  if v_redeem_quantity < 1 or v_redeem_quantity > 20 then
    raise exception 'Coupons can redeem between 1 and 20 seats.';
  end if;
  ...
  select coalesce(sum(c.redeem_quantity), 0)::int
  ...
  v_total_reserved := v_count * v_redeem_quantity;

  if v_total_reserved > v_available then
...
grant execute on function public.create_ticket_coupons (uuid, uuid, text[], text[], int) to authenticated;
...
begin
  ...
  if v_coupon.redeem_quantity < 1 or v_coupon.redeem_quantity > 20 then
    raise exception 'This coupon has an invalid redemption quantity.';
  end if;
