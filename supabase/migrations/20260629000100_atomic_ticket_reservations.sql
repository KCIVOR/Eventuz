-- Atomic ticket capacity reservation.
--
-- This moves the "check remaining capacity + create/update hold" step into the
-- database and locks the ticket type row, so concurrent buyers cannot both
-- reserve the same final slots.
...
begin
  ...
  if p_pricing_type not in ('regular', 'early_bird') then
    raise exception 'Invalid pricing type.';
  end if;

  if p_unit_price_locked is null or p_unit_price_locked < 0 then
  ...
  if v_existing_order_id is not null then
    ...
    set
      ...
      quantity = p_quantity,
      unit_price_locked = p_unit_price_locked,
      total_amount = p_total_amount,
      pricing_type = p_pricing_type,
      status = 'capacity_held',
      capacity_hold_expires_at = p_capacity_hold_expires_at,
      payment_expires_at = p_payment_expires_at,
      early_bird_price_expires_at = p_early_bird_price_expires_at
    ...
    values (
      ...
      p_pricing_type,
      'capacity_held',
      p_capacity_hold_expires_at,
      p_payment_expires_at,
      p_early_bird_price_expires_at
    )
    returning id into v_order_id;
